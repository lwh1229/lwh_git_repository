...
*/
// Registration Visualization  iss3d_Shot_Ransac_Icp::
...
double computeCloudResolution(const pcl::PointCloud<PointT>::ConstPtr& cloud)
{
    ...
    {
        if (!std::isfinite((*cloud)[i].x))  //pcl_isfinite更换std::isfinite
        {
            continue;
        }
...
int main() 
//iss3d_Shot_Ransac_Icp::fpip_ISS3D_CSHOT_RANSAC_ICP()
{
    ...
    voxel_grid_src.setLeafSize(0.005f,0.005f, 0.005f);
    ...
    voxel_grid_tar.setInputCloud(rCloud_tar);
    voxel_grid_tar.setLeafSize(0.005f,0.005f, 0.005f);
