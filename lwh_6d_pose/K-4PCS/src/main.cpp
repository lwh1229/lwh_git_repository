...
main(int argc, char** argv)
{
	...
	if (pcl::io::loadPCDFile<pcl::PointXYZ>("../../6d_data/UWA/model/parasaurolophus_low.pcd", *target) == -1)// 加载目标点云
	{
		PCL_ERROR("读取目标点云失败 \n");
		return (-1);
	}
	...
	if (pcl::io::loadPCDFile<pcl::PointXYZ>("../../6d_data/UWA/scene/rs1_0.pcd", *source) == -1)// 加载源点云
	{
		PCL_ERROR("读取源标点云失败 \n");
		return (-1);
	}
