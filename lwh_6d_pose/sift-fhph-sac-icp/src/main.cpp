...
typedef pcl::FPFHSignature33 SignT;
...
double computeCloudResolution(const pcl::PointCloud<PointT>::ConstPtr& cloud)
{
    ...
    {
        if (!std::isfinite((*cloud)[i].x)) //skipping NaNs  你可以在代码中将pcl_isfinite替换为std::isfinite以避免这个警告
        {
            continue;
        }
...
int main()
{
    ...
    */
    ...
    // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    // Feature Descriptor: compute FPFH feature descriptors
    // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    clock_t tik_descriptors_a = clock();
 
    //Calc FPFH feature descriptors
    pcl::FPFHEstimation<PointT, pcl::Normal, SignT> fpfh_src;
    //pcl::search::KdTree<PointT>::Ptr tree_src_fpfh(new pcl::search::KdTree<PointT>);
    fpfh_src.setSearchMethod(treeSiftSrc);
    fpfh_src.setInputCloud(iKeypoints_src);
    fpfh_src.setInputNormals(iCloud_src_normals);
    fpfh_src.setRadiusSearch(fpfhparm::radius_dense);
    fpfh_src.setSearchSurface(iCloud_src);
    PointCloudDesc::Ptr rFPFHDesc_src(new PointCloudDesc());
    fpfh_src.compute(*rFPFHDesc_src);
    std::cout << "------> Computed *rFPFHDesc_src FPFH descriptor values ..." << std::endl;
    std::cout << rFPFHDesc_src->is_dense << std::endl;
    std::cout << rFPFHDesc_src->size() << std::endl;
 
    pcl::FPFHEstimation<PointT, pcl::Normal, SignT> fpfh_tar;
    //pcl::search::KdTree<PointT>::Ptr tree_tar_fpfh(new pcl::search::KdTree<PointT>);
    fpfh_tar.setSearchMethod(treeSiftTar);
    fpfh_tar.setInputCloud(iKeypoints_tar);
    fpfh_tar.setInputNormals(iCloud_tar_normals);
    fpfh_tar.setRadiusSearch(fpfhparm::radius_dense);
    fpfh_tar.setSearchSurface(iCloud_tar);
    PointCloudDesc::Ptr rFPFHDesc_tar(new PointCloudDesc());
    fpfh_tar.compute(*rFPFHDesc_tar);
    std::cout << "------> Computed *rFPFHDesc_tar FPFH descriptor values ..." << std::endl;
    std::cout << rFPFHDesc_tar->is_dense << std::endl;
    std::cout << rFPFHDesc_tar->size() << std::endl;
 
    clock_t tik_descriptors_b = clock();
 
    pcl::io::savePCDFileASCII(filename_desc_fpfh_src, *rFPFHDesc_src);
    pcl::io::savePCDFileASCII(filename_desc_fpfh_tar, *rFPFHDesc_tar);
    ...
    scia.setSourceFeatures(rFPFHDesc_src);
    scia.setTargetFeatures(rFPFHDesc_tar);
    ...
    std::cout << "------> FPFH time      : " << (double)(tik_descriptors_b - tik_descriptors_a) / (double)CLOCKS_PER_SEC << " s" << std::endl;
    ...
    fout << "------> FPFH time      : " << (double)(tik_descriptors_b - tik_descriptors_a) / (double)CLOCKS_PER_SEC << " s" << std::endl;
    ...
    std::cout << "------> Saved evaluation result '" << datastream::filename_pip_fpfh << "', run run Miumiu run...\n";
