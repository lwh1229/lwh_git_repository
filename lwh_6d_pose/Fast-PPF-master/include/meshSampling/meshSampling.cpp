...
/*函数接受文件名filename、采样点数量 num_samples、输入的叶子大小 input_leaf_size、
是否写入点云文件 write_pcd_file，以及一个 pcl::PointCloudpcl::PointXYZ::Ptr 类型的指针 cloud_out，用于存储最终生成的点云数据。

根据文件扩展名判断输入的文件类型，并使用相应的库函数加载三维模型文件到 pcl::PolygonMesh 对象 mesh 中。接着，将 mesh 转换为 vtkPolyData 对象 polydata1。
随后，通过 vtk 调用将多边形网格数据转换为三角形网格数据，并更新 polydata1。接下来，创建一个 pcl::PointCloudpcl::PointXYZRGBNormal 对象 cloud_1，
并调用函数 uniform_sampling 对 polydata1 进行均匀采样，生成点云数据。

然后，创建一个 VoxelGrid 滤波器对象 grid_，并设置输入点云数据 cloud_1 和叶子大小。
调用 filter 方法将滤波器应用于输入点云，生成一个经过 VoxelGrid 滤波器处理的点云数据 voxel_cloud。
从 voxel_cloud 中复制只包含 XYZ 坐标的部分，并存储到 pcl::PointCloudpcl::PointXYZ 对象 cloud_xyz 中。
如果指定了写入点云文件的参数 write_cloud，则将 cloud_xyz 保存为点云文件。然后，将 cloud_xyz 的数据赋值给 cloud_out。*/
